...
\newcommand{\duedate}{April 24, 2020}
...
I am attaching a binary tree source code (\texttt{bst-0.0.cpp}) with the methods insert, delete, and print. Your job would be to implement a Red-Black Tree, adapting the functions insert, remove, and print as convenient. 

To test your code, you can follow the examples described in the document \texttt{anexo1.pdf}. Also, you might be interested in the document \texttt{anexo2.pdf} for a more detailed description of this tree, there is also some Java code that might be useful. 
However, your code must be in C++ and modifying the attached code. The grading will be as follow:

\begin{enumerate}[label=(\alph*)]
  \item \textbf{(2pts)} insert 
  \item \textbf{(2pts)} remove 
\end{enumerate}
